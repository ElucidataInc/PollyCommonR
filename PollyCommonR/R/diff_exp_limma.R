...
diff_exp_limma <- function (prot_norm_mat, metadata, cohort_condition, cohort_a, 
  ...
  prot_norm_mat_log2 <- prot_norm_mat_log2[!is.infinite(rowSums(prot_norm_mat_log2)), ]
  ...
  if (length(cohort_a_columns) <= 1 | length(cohort_b_columns) <= 1) {
    ...
    limma_results_df <- limma::topTable(fit, coef = paste(cohort_b, 
                                                          cohort_a, sep = "-"), number = nrow(prot_norm_mat_log2))
    ...
    limma_results_df <- limma_results_df[!apply(limma_results_df, 
                                                1, anyNA), ]
    limma_results_df <- limma_results_df[!is.infinite(rowSums(limma_results_df)), 
                                         ]
    limma_results_df <- limma_results_df[with(limma_results_df, 
                                              order(-logFC)), ]
